...
const ExportModal = ({ isOpen, onClose, onExport, currentTheme = 'dark' }) => {
    ...
    const handleExportClick = async () => {
        ...
        if (exportType === 'all') {
            ...
            alert(t('export.startDateAfterEndDate'));
    ...
    return (
        <div className={`fixed inset-0 ${currentTheme === 'dark' ? 'bg-black bg-opacity-75' : 'bg-black bg-opacity-50'} flex items-center justify-center z-50 p-4`}>
            <div className={`${currentTheme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-2xl p-6 w-full max-w-md`}>
                <h2 className={`text-2xl font-bold ${currentTheme === 'dark' ? 'text-white' : 'text-gray-900'} mb-4`}>{t('export.title')}</h2>
                <div className="space-y-4">
                    <div className="flex items-center gap-4">
                        <input type="radio" id="all" name="exportType" value="all" checked={exportType === 'all'} onChange={() => setExportType('all')} />
                        <label htmlFor="all" className={currentTheme === 'dark' ? 'text-white' : 'text-gray-900'}>{t('export.exportAll')}</label>
                    </div>
                    <div className="flex items-center gap-4">
                        <input type="radio" id="range" name="exportType" value="range" checked={exportType === 'range'} onChange={() => setExportType('range')} />
                        <label htmlFor="range" className={currentTheme === 'dark' ? 'text-white' : 'text-gray-900'}>{t('export.selectPeriod')}</label>
                    </div>
                    {exportType === 'range' && (
                        <div className="pl-8 flex flex-col sm:flex-row gap-4">
                            <div className="flex-1">
                                <label htmlFor="start-date-export" className={`block text-sm font-medium ${currentTheme === 'dark' ? 'text-gray-300' : 'text-gray-700'} mb-1`}>{t('export.from')}</label>
                                <input 
                                    type="date" 
                            ...
                            <div className="flex-1">
                                <label htmlFor="end-date-export" className={`block text-sm font-medium ${currentTheme === 'dark' ? 'text-gray-300' : 'text-gray-700'} mb-1`}>{t('export.to')}</label>
                                <input 
                                    type="date" 
                                    id="end-date-export" 
                                    value={endDate} 
